...
ry_intervals = {
        ...
        '7e'  : {'name' : 'septième', 'fraction' : 8/15, 'color' : '#17becf', 'name_EN' : '7th'},
        ...
        '2de min' : {'name' : 'seconde mineure', 'fraction' : 15/16, 'color' : '#e377c2', 'name_EN' : 'min 2nd'},
...
#    spec_cmap = 'BuPu'
    ...
    def __init__ (self, filename, frame_size = 256, tightness = 100, beat_frame_offset = 0):
        ...
        self.delta = 1
        self.display_rychords_inv = False
        self.display_rychords_log = False
    ...
    def amplify (self):
        self.signal *= self.amplification
        return None
    ...
    def inverse_rychords (self):
        rychords_inv = ()
        for rychord in self.rychords:
            rynter_inv = ()
            for rynter in rychord:
                if rynter != 0:
                    rynter_inv += 1 / rynter,
                else:
                    rynter_inv += -1,
            rychords_inv += rynter_inv,
                
        self.rychords_inv = rychords_inv
        return None
    
    
    def rychords_max (self, rychords):
        fractions = ()
        for rychord in rychords:
            if len (rychord) == 1:
                fractions += rychord [0],
            else:
                for rynter in rychord:
    ...
    def display_rhythm_chords (self, ax1, inverse = False , hide_unitones = True, eps = 0.05, show_rynterval_names = False):        
        # build chords 
        rychords_hist = ()
        if inverse:
            self.inverse_rychords ()
            rychords = self.rychords_inv
        else:
            rychords = self.rychords
            
        for i in range (len (self.time_rychords)):
            for j in range (len (rychords [i])):
                if rychords [i][j] > 0:
                    if ((hide_unitones and eps <= self.rychords [i][j] <= 1 - eps) or
                        not hide_unitones):
                        ax1.scatter (self.time_rychords [i] * self.frame_size / self.samplerate,
                                     rychords [i][j], marker = 'o', color = self.wavecolor)
                        rychords_hist += rychords [i][j],
        ...
        # display background ryntervals    
        max_rynter = self.rychords_max (rychords)
        power = (-1) ** (inverse)
        for interval in ry_intervals:
            ax1.hlines (ry_intervals [interval]['fraction'] ** power,
            ...
            if (inverse and max_rynter > 2) or not inverse:
                ax1.hlines ((ry_intervals [interval]['fraction'] / 2 ) ** power,
                            0, xmax, color = ry_intervals [interval]['color'], 
                            ls = '-', lw = 1.2, alpha = 0.4)
            if (inverse and max_rynter > 4) or not inverse:
                ax1.hlines ((ry_intervals [interval]['fraction'] / 4) ** power,
                            0, xmax, color = ry_intervals [interval]['color'], 
                            ls = '-', lw = 1.2, alpha = 0.2)    

        ylim = ax1.get_ylim ()
        for i in range (len (self.time_rychords)):
            ax1.vlines (self.time_rychords [i] * self.frame_size / self.samplerate,
                        ylim [0], ylim [1],
                        color = 'k', ls = 'dotted', alpha = 0.15)
        
        if not self.display_rychords_inv and not self.display_rychords_log: 
            ax1.set_ylim (0,1)
        
        if self.display_rychords_log:
            plt.yscale ('log')
    ...
    def triple_analyse (self, ax, DEBUG = False):
        ...
        self.shift_beat_frame (self.beat_frame_shift)
        ...
        self.display_rhythm_chords (ax3,
                                    inverse = self.display_rychords_inv,
...
def main ():
    ...
    path2source = '/home/joseph/Collab_Lasayah/RythmListener/sources/AUDIO/youtube-dl/extraits/'
...
#    piece.calculate_onset_consonnace ()
    piece.amplification = 3
    piece.tempo_factor = 0.5
    piece.beat_frame_shift = 0.5
    piece.tightness = 200
    piece.delta = 0.08
    piece.hide_unitones = True
    piece.display_rychords_inv = False
    piece.show_rynterval_names = False
    piece.unitone_deviation = 0.11
    piece.display_rychords_log = True
    ...
    piece.display_rychords_inv = False
    piece.display_rychords_log = False
    fig, ax = plt.subplots (nrows = 3, ncols = 1, figsize = figure_size)
    plt.subplots_adjust (hspace = None)
    piece.triple_analyse (ax)   

#    piece.display_rhythm_chords (ax,
#                                 inverse = piece.display_rychords_inv,
#                                 hide_unitones = piece.display_rychords_inv,
#                                 eps = piece.unitone_deviation,
#                                 show_rynterval_names = piece.show_rynterval_names)
#    
#    plt.yscale ('log')
